...
<category>` create files with the right structure and `TODO` markers you must
  fill in before lint passes.
...
PostCSS with `postcss-import` resolves the bare specifier through the `style`
field; only `exports`-aware bundlers (Vite, webpack) resolve `./styles.css`.
