...
export function buildOracle(
  ...
  function measure(layer, path, parent, far) {
    ...
    // A line is its stroke, the top edge of a box as long as the line and as tall as the stroke
    // (recipe.mjs), not Figma's outline of it, which stops where Figma's sample line does.
    const line = layer.type === 'LINE' && layer.strokes?.length > 0;
    // The shape drawn, as Figma's path data, compared as data by the visual checks.
    if (!line && (layer.geometry || layer.strokeGeometry))
      out.glyph = {
    ...
    if (line)
      ['Top', 'Right', 'Bottom', 'Left'].forEach((side) => {
        out[`border${side}Width`] = side === 'Top' ? layer.strokeWeight : 0;
      });
    else if (layer.strokes?.length && layer.strokeWeights)
    ...
    // A dashed edge's pattern, each dash's length and the gap after it (FileUpload's drop zone).
    if (layer.strokes?.length && layer.dashes?.length)
      out.borderDash = [...layer.dashes];
    ...
    if (line && 'height' in out) out.height = layer.strokeWeight;
    return { out, unresolved };
  }
  ...
  // A layer one variant dashes and another draws solid (Image Card's empty state, dashed): the
  // solid one says so, so a renderer that leaves the dashes on is caught as well.
  const dashed = new Set(
    variants.flatMap((v) =>
      Object.entries(v.layers).flatMap(([name, l]) =>
        l.borderDash ? [name] : [],
      ),
    ),
  );
  for (const v of variants)
    for (const name of dashed) {
      const l = v.layers[name];
      if (l && !l.borderDash && !l.hidden) l.borderDash = false;
    }
