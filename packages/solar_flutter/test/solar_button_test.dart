...
void main() {
  group('SolarButton', () {
    ...
    testWidgets('secondary loading shows the default spinner, lg the md one', (
      ...
      expect(
        tester.widget<SolarSpinner>(find.byType(SolarSpinner)).size,
        SolarSpinnerSize.md,
      );
    });
  ...
  group('SolarSpinner', () {
    ...
    testWidgets('inverse uses the inverse border colours', (tester) async {
      await pump(
        tester,
        const SolarSpinner(variant: SolarSpinnerVariant.inverse),
      );
      final ring = tester.widget<CircularProgressIndicator>(
        find.byType(CircularProgressIndicator),
      );
    ...
    testWidgets(
      'follows the app’s brightness when no SolarTheme is installed',
      (tester) async {
