// SOLAR FAB recipe for Flutter. Generated by @bwp-web/codegen from spec/components/fab.json. Do not edit.
//
// The recipe is data: token *names*, keyed by layer and cell and by the base, size, appearance
// and combined sections of the IR. Values resolve against the ambient SolarTheme, so Light and
// Dark switch with the theme and no colour here is a literal.

import 'package:flutter/foundation.dart' show immutable;
import 'package:flutter/material.dart'
    show
        BorderRadius,
        BorderSide,
        BoxDecoration,
        BoxShadow,
        ButtonStyle,
        Color,
        Colors,
        DecoratedBox,
        EdgeInsetsDirectional,
        NoSplash,
        RoundedRectangleBorder,
        Size,
        TextStyle,
        VisualDensity,
        WidgetState,
        WidgetStateProperty,
        WidgetStatePropertyAll;

import '../tokens.dart';

enum SolarFABSize { sm, md }

enum SolarFABType { icon, extended }

/// The props a SOLAR FAB takes. Hover, pressed and focus are not here: they are
/// platform states, tracked by Flutter as [WidgetState]s.
@immutable
class SolarFABProps {
  const SolarFABProps({
    this.size = SolarFABSize.sm,
    this.disabled = false,
    this.loading = false,
    this.type = SolarFABType.icon,
  });

  final SolarFABSize size;
  final bool disabled;
  final bool loading;
  final SolarFABType type;
}

abstract final class SolarFABRecipe {
  /// Every IR entry, as `<layer>.<cell>|<section>…` to a token name (`t:`), a keyword (`k:`),
  /// `none`, a presence (`b:`) or a literal the overlay allowed (`px:`).
  static const Map<String, String> cells = {
    'root.present|base': 'b:true',
    'root.background|base': 't:color.action.primary.bg.default',
    'root.borderColor|base': 't:color.border.medium',
    'root.shadow|base': 't:shadow.overlay',
    'root.radius|base': 't:radius.pill',
    'root.borderWidth|base': 't:border.default',
    'root.direction|base': 'k:HORIZONTAL',
    'root.align|base': 'k:CENTER/CENTER',
    'root.gap|base': 't:inset.none',
    'root.paddingTop|base': 't:inset.sm',
    'root.paddingRight|base': 't:inset.sm',
    'root.paddingBottom|base': 't:inset.sm',
    'root.paddingLeft|base': 't:inset.sm',
    'root.width|base': 'px:44',
    'root.height|base': 'px:44',
    'root.paddingTop|size|md': 't:inset.md',
    'root.paddingBottom|size|md': 't:inset.md',
    'root.paddingLeft|size|md': 't:inset.md',
    'root.height|size|md': 'px:56',
    'root.background|appearance|type=icon|hover':
        't:color.action.primary.bg.hover',
    'root.background|appearance|type=icon|pressed':
        't:color.action.primary.bg.active',
    'root.background|appearance|type=icon|disabled':
        't:color.action.primary.bg.disabled',
    'root.shadow|appearance|type=icon|focus': 't:shadow.focus.default',
    'root.background|appearance|type=extended|hover':
        't:color.action.primary.bg.hover',
    'root.background|appearance|type=extended|pressed':
        't:color.action.primary.bg.active',
    'root.background|appearance|type=extended|disabled':
        't:color.action.primary.bg.disabled',
    'root.shadow|appearance|type=extended|focus': 't:shadow.focus.default',
    'root.gap|combined|sm|type=extended|default': 't:stack.xs',
    'root.paddingRight|combined|sm|type=extended|default': 't:inset.lg',
    'root.width|combined|sm|type=extended|default': 'k:HUG',
    'root.background|combined|sm|type=extended|focus':
        't:color.action.primary.bg.default',
    'root.background|combined|sm|type=icon|focus':
        't:color.action.primary.bg.default',
    'root.gap|combined|md|type=extended|default': 't:stack.sm',
    'root.paddingRight|combined|md|type=extended|default': 't:inset.xl',
    'root.width|combined|md|type=extended|default': 'k:HUG',
    'root.background|combined|md|type=extended|focus':
        't:color.action.primary.bg.default',
    'root.paddingRight|combined|md|type=icon|default': 't:inset.md',
    'root.width|combined|md|type=icon|default': 'px:56',
    'root.background|combined|md|type=icon|focus':
        't:color.action.primary.bg.default',
    'icon.present|base': 'b:true',
    'icon.component|base': 'k:Icon/None',
    'icon.variant.solid|base': 'k:false',
    'icon.width|base': 't:icon.md',
    'icon.height|base': 't:icon.md',
    'icon.color|base': 't:color.action.primary.icon.default',
    'icon.present|appearance|type=icon|loading': 'b:false',
    'icon.color|appearance|type=icon|hover':
        't:color.action.primary.icon.hover',
    'icon.color|appearance|type=icon|pressed':
        't:color.action.primary.icon.active',
    'icon.color|appearance|type=icon|disabled':
        't:color.action.primary.icon.disabled',
    'icon.present|appearance|type=extended|loading': 'b:false',
    'icon.color|appearance|type=extended|hover':
        't:color.action.primary.icon.hover',
    'icon.color|appearance|type=extended|pressed':
        't:color.action.primary.icon.active',
    'icon.color|appearance|type=extended|disabled':
        't:color.action.primary.icon.disabled',
    'icon.width|combined|md|type=extended|default': 't:icon.lg',
    'icon.height|combined|md|type=extended|default': 't:icon.lg',
    'icon.color|combined|md|type=extended|focus':
        't:color.action.primary.icon.default',
    'icon.color|combined|md|type=icon|focus':
        't:color.action.primary.icon.default',
    'icon.color|combined|sm|type=icon|focus':
        't:color.action.primary.icon.default',
    'icon.color|combined|sm|type=extended|focus':
        't:color.action.primary.icon.default',
    'spinner.present|base': 'b:false',
    'spinner.component|base': 'k:Spinner',
    'spinner.variant.size|base': 'k:sm',
    'spinner.width|base': 'k:HUG',
    'spinner.height|base': 'k:HUG',
    'spinner.variant.size|size|md': 'k:md',
    'spinner.present|appearance|type=icon|loading': 'b:true',
    'spinner.variant.style|appearance|type=icon|loading': 'k:inverse',
    'spinner.present|appearance|type=extended|loading': 'b:true',
    'spinner.variant.style|appearance|type=extended|loading': 'k:inverse',
    'label.present|base': 'b:false',
    'label.typography|base': 't:typography.label.sm',
    'label.typography|size|md': 't:typography.label.md',
    'label.present|appearance|type=extended|default': 'b:true',
    'label.color|appearance|type=extended|default':
        't:color.action.primary.text.default',
    'label.present|appearance|type=extended|hover': 'b:true',
    'label.color|appearance|type=extended|hover':
        't:color.action.primary.text.default',
    'label.present|appearance|type=extended|pressed': 'b:true',
    'label.color|appearance|type=extended|pressed':
        't:color.action.primary.text.default',
    'label.present|appearance|type=extended|disabled': 'b:true',
    'label.color|appearance|type=extended|disabled':
        't:color.action.primary.text.disabled',
    'label.present|appearance|type=extended|focus': 'b:true',
    'label.color|appearance|type=extended|focus':
        't:color.action.primary.text.default',
    'label.present|appearance|type=extended|loading': 'b:false',
    'label.color|appearance|type=extended|loading':
        't:color.action.primary.text.default',
  };

  /// Which state wins when several hold, highest first: the MUI recipe's cascade, read backwards.
  static const List<String> statePrecedence = [
    'disabled',
    'loading',
    'focus',
    'pressed',
    'hover',
  ];

  static bool _holds(String state, SolarFABProps p, Set<WidgetState> s) =>
      switch (state) {
        'disabled' =>
          p.disabled || (!p.loading && s.contains(WidgetState.disabled)),
        'loading' => p.loading,
        'focus' => s.contains(WidgetState.focused),
        'pressed' => s.contains(WidgetState.pressed),
        'hover' => s.contains(WidgetState.hovered),
        _ => false,
      };

  /// The entry for one cell under these props and states, by the IR's precedence: a state that
  /// holds beats the resting value, the per-size-and-appearance entry beats the per-appearance
  /// one, and the resting value falls back through appearance, size and base.
  static String? lookup(String cell, SolarFABProps p, Set<WidgetState> s) {
    final combo = 'type=${p.type.name}';
    final size = p.size.name;
    for (final state in statePrecedence) {
      if (!_holds(state, p, s)) continue;
      final hit =
          cells['$cell|combined|$size|$combo|$state'] ??
          cells['$cell|appearance|$combo|$state'];
      if (hit != null) return hit;
    }
    return cells['$cell|combined|$size|$combo|default'] ??
        cells['$cell|appearance|$combo|default'] ??
        cells['$cell|size|$size'] ??
        cells['$cell|base'];
  }

  static Color color(
    SolarTheme t,
    String cell,
    SolarFABProps p,
    Set<WidgetState> s,
  ) {
    final c = t.colors;
    return switch (lookup(cell, p, s)) {
      'none' => Colors.transparent,
      't:color.action.primary.bg.active' => c.actionPrimaryBgActive,
      't:color.action.primary.bg.default' => c.actionPrimaryBgDefault,
      't:color.action.primary.bg.disabled' => c.actionPrimaryBgDisabled,
      't:color.action.primary.bg.hover' => c.actionPrimaryBgHover,
      't:color.action.primary.icon.active' => c.actionPrimaryIconActive,
      't:color.action.primary.icon.default' => c.actionPrimaryIconDefault,
      't:color.action.primary.icon.disabled' => c.actionPrimaryIconDisabled,
      't:color.action.primary.icon.hover' => c.actionPrimaryIconHover,
      't:color.action.primary.text.default' => c.actionPrimaryTextDefault,
      't:color.action.primary.text.disabled' => c.actionPrimaryTextDisabled,
      't:color.border.medium' => c.borderMedium,
      final v => throw StateError('$cell: no colour for $v'),
    };
  }

  static List<BoxShadow> shadow(
    SolarTheme t,
    String cell,
    SolarFABProps p,
    Set<WidgetState> s,
  ) => switch (lookup(cell, p, s)) {
    'none' || null => const <BoxShadow>[],
    't:shadow.focus.default' => t.shadows.focusDefault,
    't:shadow.overlay' => t.shadows.overlay,
    final v => throw StateError('$cell: no shadow for $v'),
  };

  /// A length in logical pixels, or null where the IR says the layer hugs its content.
  static double? dimension(String cell, SolarFABProps p, Set<WidgetState> s) {
    final v = lookup(cell, p, s);
    if (v == null || v == 'k:HUG') return null;
    if (v == 'k:FILL') return double.infinity;
    if (v.startsWith('px:')) return double.parse(v.substring(3));
    return switch (v) {
      't:border.default' => SolarBorder.$default,
      't:icon.lg' => SolarIconSize.lg,
      't:icon.md' => SolarIconSize.md,
      't:inset.lg' => SolarInset.lg,
      't:inset.md' => SolarInset.md,
      't:inset.none' => SolarInset.none,
      't:inset.sm' => SolarInset.sm,
      't:inset.xl' => SolarInset.xl,
      't:radius.pill' => SolarRadius.pill,
      't:stack.sm' => SolarStack.sm,
      't:stack.xs' => SolarStack.xs,
      _ => throw StateError('$cell: no length for $v'),
    };
  }

  static TextStyle? textStyle(
    SolarTheme t,
    String cell,
    SolarFABProps p,
    Set<WidgetState> s,
  ) => switch (lookup(cell, p, s)) {
    'none' || null => null,
    't:typography.label.md' => t.typography.labelMd,
    't:typography.label.sm' => t.typography.labelSm,
    final v => throw StateError('$cell: no text style for $v'),
  };

  /// Whether a layer is drawn: the shell reads this, the style does not.
  static bool present(String layer, SolarFABProps p, Set<WidgetState> s) =>
      lookup('$layer.present', p, s) != 'b:false';

  static BorderSide _side(SolarTheme t, SolarFABProps p, Set<WidgetState> s) {
    final width = lookup('root.borderWidth', p, s);
    if (width == null || width == 'none') return BorderSide.none;
    return BorderSide(
      color: color(t, 'root.borderColor', p, s),
      width: dimension('root.borderWidth', p, s)!,
    );
  }

  static BorderRadius _radius(SolarFABProps p, Set<WidgetState> s) =>
      BorderRadius.circular(dimension('root.radius', p, s) ?? 0);

  /// A [ButtonStyle] for a FilledButton that draws SOLAR's FAB.
  ///
  /// The background and the shadows are painted by one [BoxDecoration] in [ButtonStyle.backgroundBuilder]:
  /// ButtonStyle has elevation but no box shadows, and a Flutter shadow paints under the whole
  /// box, so drawing it over a separately coloured Material would darken the button's face, which
  /// CSS never does. Material's own overlay, splash and density are switched off, because every
  /// SOLAR state has its own explicit colours and sizes.
  static ButtonStyle style(
    SolarTheme t, [
    SolarFABProps p = const SolarFABProps(),
  ]) {
    WidgetStateProperty<T> by<T>(T Function(Set<WidgetState> s) f) =>
        WidgetStateProperty.resolveWith(f);
    const none = <WidgetState>{};
    final width = dimension('root.width', p, none);
    final height = dimension('root.height', p, none);
    return ButtonStyle(
      backgroundColor: const WidgetStatePropertyAll(Colors.transparent),
      foregroundColor: by(
        (s) => lookup('label.color', p, s) == null
            ? null
            : color(t, 'label.color', p, s),
      ),
      iconColor: by(
        (s) => lookup('icon.color', p, s) == null
            ? null
            : color(t, 'icon.color', p, s),
      ),
      iconSize: WidgetStatePropertyAll(dimension('icon.width', p, none)),
      textStyle: by((s) => textStyle(t, 'label.typography', p, s)),
      padding: by(
        (s) => EdgeInsetsDirectional.only(
          start: dimension('root.paddingLeft', p, s) ?? 0,
          end: dimension('root.paddingRight', p, s) ?? 0,
          top: dimension('root.paddingTop', p, s) ?? 0,
          bottom: dimension('root.paddingBottom', p, s) ?? 0,
        ),
      ),
      minimumSize: WidgetStatePropertyAll(Size(width ?? 0, height ?? 0)),
      fixedSize: width == null
          ? null
          : WidgetStatePropertyAll(Size(width, height ?? 0)),
      side: by((s) => _side(t, p, s)),
      shape: by((s) => RoundedRectangleBorder(borderRadius: _radius(p, s))),
      elevation: const WidgetStatePropertyAll(0),
      shadowColor: const WidgetStatePropertyAll(Colors.transparent),
      surfaceTintColor: const WidgetStatePropertyAll(Colors.transparent),
      overlayColor: const WidgetStatePropertyAll(Colors.transparent),
      splashFactory: NoSplash.splashFactory,
      visualDensity: VisualDensity.standard,
      backgroundBuilder: (context, s, child) => DecoratedBox(
        decoration: BoxDecoration(
          color: color(t, 'root.background', p, s),
          borderRadius: _radius(p, s),
          boxShadow: shadow(t, 'root.shadow', p, s),
        ),
        child: child,
      ),
    );
  }
}
