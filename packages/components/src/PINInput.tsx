 ...
 * `solarPINInputTree` and `solarPINInputSlots` beside the recipe. What it looks like is not here.
 * That is the recipe, `solarPINInputStyle` and `solarPINInputCompose` in `@bwp-web/styles/mui`:
 * each cell's fill, edge and focus ring by state, its digit's and placeholder's ink, and the label,
 * helper and error.
 *
...
const CELLS = solarPINInputTree.cells;

/** A cell's placeholder and digit, as Figma names them. */
const partsOf = (cell: string) => ({
  placeholder: solarPINInputTree[cell].find((l) =>
    l.startsWith('placeholder'),
  )!,
  digit: solarPINInputTree[cell].find((l) => l.startsWith('digit'))!,
});
...
export const PINInput = forwardRef<HTMLDivElement, PINInputProps>(
  function PINInput(inProps, ref) {
    ...
    const drawing: LayerDrawing = {
      ...
      tree: solarPINInputTree,
      slots: solarPINInputSlots,
      parts: {
        ...parts,
        ...Object.fromEntries(
