 ...
 * SOLAR Divider.
 *
 * Scaffolded once by `npm run solar:scaffold Divider` from spec/components/divider.json, and owned
 * by developers from then on: change it freely. What it looks like is not here. That is the recipe,
 * `solarDividerStyle` and `solarDividerCompose` in `@bwp-web/styles/mui`: the rule’s colour and
 * thickness, the label’s text style, and each type’s gap and inset.
 *
