 ...
 * `solarSplitButtonTree` and `solarSplitButtonSlots` beside the recipe. What it looks like is not
 * here. That is the recipe, `solarSplitButtonStyle` in `@bwp-web/styles/mui`: the control's
 * colours, border, shadow and focus ring by state, its halves' padding, and the rule between them.
 *
...
import {
  ...
  solarSplitButtonSlots,
  solarSplitButtonTree,
    ...
    return (
      <>
        <Box
          ...
          {...rest}
          sx={[
            solarSplitButtonStyle({ prio, size, disabled, loading }),
            ...(Array.isArray(sx) ? sx : [sx]),
          ]}
        >
          {drawChildren('root', {
            prefix: 'SolarSplitButton',
            tree: solarSplitButtonTree,
            slots: solarSplitButtonSlots,
            parts: kept,
            text: { label: children },
