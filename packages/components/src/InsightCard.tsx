 ...
 * `solarInsightCardTree` and `solarInsightCardSlots` beside the recipe. What it looks like is not
 * here. That is the recipe, `solarInsightCardStyle` and `solarInsightCardCompose` in
 * `@bwp-web/styles/mui`: the surface’s fill, edge, shadow and focus ring by state, the severity’s
 * tile and More ink, its words, the StatusIndicator it shows, and the placeholders.
 *
...
import {
  ...
  solarInsightCardSlots,
  solarInsightCardTree,
...
export const InsightCard = forwardRef<HTMLDivElement, InsightCardProps>(
  function InsightCard(inProps, ref) {
    // As the app's MUI theme sets them (components.SolarInsightCard), under the caller's own.
    const {
      severity = 'success',
      selected = false,
      loading = false,
      ...
      sx,
      ...rest
    } = useSolarProps(inProps, 'SolarInsightCard');
      ...
      <>
        ...
        >
          ...
          {drawChildren('root', {
            ...
            tree: solarInsightCardTree,
            slots: solarInsightCardSlots,
            parts,
