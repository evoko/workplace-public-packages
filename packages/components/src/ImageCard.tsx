 ...
 * `solarImageCardTree` and `solarImageCardSlots` beside the recipe. What it looks like is not here.
 * That is the recipe, `solarImageCardStyle` and `solarImageCardCompose` in `@bwp-web/styles/mui`:
 * the tile's fill, edge, shadow and focus ring, the image's frame, its words' ink, the Checkbox it
...
export const ImageCard = forwardRef<HTMLDivElement, ImageCardProps>(
  function ImageCard(inProps, ref) {
    // As the app's MUI theme sets them (components.SolarImageCard), under the caller's own.
    const {
      filled = true,
      selected = false,
      title,
      ...
      onBlur,
      ...rest
    } = useSolarProps(inProps, 'SolarImageCard');
      ...
      <>
        ...
        >
          {drawChildren('root', {
            ...
            tree: solarImageCardTree,
            slots: solarImageCardSlots,
            parts,
