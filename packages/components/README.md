...
| `OrganizationJoinPanel`          | Single-field form card for the join flow, sized to replace `OrganizationsPanel` in place; `submitting` shows a spinner and blocks double-submit, `slotProps` reaches label/field/buttons                      |
| `OrganizationCreatePanel`        | Region dropdown, name, domain and a checkbox for the create flow, on the same card as `OrganizationsPanel`; `submitting` shows a spinner and blocks double-submit, `slotProps` reaches every field and button |
