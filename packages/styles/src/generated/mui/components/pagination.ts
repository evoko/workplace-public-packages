// SOLAR Pagination recipe for MUI. Generated by @bwp-web/codegen from spec/components/pagination.json. Do not edit.
// Plain data on purpose: this module imports nothing, so @bwp-web/styles stays dependency free.
// Every value is a var(--solar-*) reference into tokens.css, which must be loaded.

export type SolarPaginationProps = Record<never, never>;

export const solarPaginationDefaults = {} as const;

/** Style by layer and state: `root` is the base, then per size, per appearance, and per size and appearance together. */
export const solarPaginationStyles = {
  reset: {
    display: 'flex',
    position: 'relative',
    boxSizing: 'border-box',
    flexShrink: '0',
    '& .SolarPagination-box': {
      display: 'flex',
      boxSizing: 'border-box',
    },
    '& .SolarPagination-text': {
      whiteSpace: 'nowrap',
    },
    '& .SolarPagination-glyph': {
      display: 'block',
      overflow: 'visible',
      flexShrink: '0',
    },
    '& .SolarPagination-drawnIcon': {
      flexShrink: '0',
    },
    '& .SolarGlyph-fill, & .SolarGlyph-stroke': {
      stroke: 'none',
    },
    '& .SolarPagination-list': {
      display: 'flex',
      gap: 'inherit',
      alignItems: 'inherit',
      margin: '0',
      padding: '0',
      listStyle: 'none',
    },
    '& .SolarPagination-list > li': {
      display: 'flex',
    },
  },
  root: {
    backgroundColor: 'transparent',
    borderColor: 'transparent',
    boxShadow: 'none',
    borderRadius: 'var(--solar-radius-none)',
    borderStyle: 'none',
    flexDirection: 'row',
    justifyContent: 'center',
    alignItems: 'center',
    gap: 'var(--solar-stack-xs)',
    paddingTop: 'var(--solar-inset-none)',
    paddingRight: 'var(--solar-inset-none)',
    paddingBottom: 'var(--solar-inset-none)',
    paddingLeft: 'var(--solar-inset-none)',
  },
  sizes: {},
  appearances: {},
  combined: {},
} as const;

/** What the shell renders rather than styles: which layers show, and which variant each child takes. */
export const solarPaginationComposition = {
  root: {
    base: {
      present: true,
    },
  },
  previous: {
    base: {
      present: true,
      component: 'PaginationNav',
      'variant.direction': 'previous',
      'variant.state': 'disabled',
    },
  },
  page1: {
    base: {
      present: true,
      component: 'PaginationItem',
      'variant.selected': 'true',
      'variant.state': 'default',
    },
  },
  page2: {
    base: {
      present: true,
      component: 'PaginationItem',
      'variant.selected': 'false',
      'variant.state': 'default',
    },
  },
  page3: {
    base: {
      present: true,
      component: 'PaginationItem',
      'variant.selected': 'false',
      'variant.state': 'default',
    },
  },
  paginationEllipsis: {
    base: {
      present: true,
      component: 'PaginationEllipsis',
    },
  },
  page12: {
    base: {
      present: true,
      component: 'PaginationItem',
      'variant.selected': 'false',
      'variant.state': 'default',
    },
  },
  next: {
    base: {
      present: true,
      component: 'PaginationNav',
      'variant.direction': 'next',
      'variant.state': 'default',
    },
  },
} as const;

type Style = { [key: string]: string | Style };

function merge(...styles: (Style | undefined)[]): Style {
  const out: Style = {};
  for (const style of styles)
    for (const [k, v] of Object.entries(style ?? {}))
      out[k] =
        typeof v === 'object' && typeof out[k] === 'object'
          ? merge(out[k] as Style, v)
          : v;
  return out;
}

/** The complete style for one set of props, for `sx` or `styleOverrides.root`. */
export function solarPaginationStyle(props: SolarPaginationProps = {}): Style {
  const p: Record<string, unknown> = { ...solarPaginationDefaults };
  for (const [k, v] of Object.entries(props)) if (v !== undefined) p[k] = v;
  const key = `default`;
  const s = solarPaginationStyles as unknown as {
    reset: Style;
    root: Style;
    sizes: Record<string, Style>;
    appearances: Record<string, Style>;
    combined: Record<string, Record<string, Style>>;
  };
  return merge(s.reset, s.root, s.appearances[key]);
}

/** A drawn layer's outline: its box, and the fill's and the stroke's paths (Figma's geometry). */
type Glyph = {
  width: number;
  height: number;
  fill: { d: string; evenOdd: boolean }[];
  stroke: { d: string; evenOdd: boolean }[];
};
/** One layer's composition: shown or not, its composed child, its glyph, its position. */
export type SolarPaginationParts = Record<
  string,
  string | number | boolean | Glyph | null
>;
type Parts = SolarPaginationParts;
type Layered = {
  base: Parts;
  size?: Record<string, Parts>;
  appearance?: Record<string, Record<string, Parts>>;
  combined?: Record<string, Record<string, Record<string, Parts>>>;
};

/**
 * What the shell renders for one set of props in one state, by layer: whether it shows, and which
 * component and variant a composed child takes. The same precedence as the style: base, size,
 * appearance, then size and appearance together, each at rest and then in the state.
 */
export function solarPaginationCompose(
  props: SolarPaginationProps = {},
  state: string = 'default',
): Record<string, Parts> {
  const p: Record<string, unknown> = { ...solarPaginationDefaults };
  for (const [k, v] of Object.entries(props)) if (v !== undefined) p[k] = v;
  const key = `default`;
  const size = '';
  const out: Record<string, Parts> = {};
  for (const [layer, c] of Object.entries(
    solarPaginationComposition as unknown as Record<string, Layered>,
  )) {
    const states = state === 'default' ? ['default'] : ['default', state];
    out[layer] = Object.assign(
      {},
      c.base,
      c.size?.[size],
      ...states.flatMap((st) => [
        c.appearance?.[key]?.[st],
        c.combined?.[size]?.[key]?.[st],
      ]),
    );
  }
  return out;
}

/** Each layer's children, in Figma's order: the tree the shell draws (`drawChildren`). */
export const solarPaginationTree: Record<string, string[]> = {
  root: [
    'previous',
    'page1',
    'page2',
    'page3',
    'paginationEllipsis',
    'page12',
    'next',
  ],
};

/** Each slot's layer, to the slot it is: its class is `SolarPagination-<slot>`, public. */
export const solarPaginationSlots: Record<string, string> = {};
