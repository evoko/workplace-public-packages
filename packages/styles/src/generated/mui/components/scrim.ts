// SOLAR Scrim recipe for MUI. Generated by @bwp-web/codegen from spec/components/scrim.json. Do not edit.
...
export type SolarScrimProps = Record<never, never>;

export const solarScrimDefaults = {} as const;

/** Style by layer and state: `root` is the base, then per size, per appearance, and per size and appearance together. */
export const solarScrimStyles = {
  reset: {},
  root: {
    backgroundColor: 'var(--solar-color-surface-scrim)',
...
export const solarScrimComposition = {
  root: {
    base: {
      present: true,
    },
...
export function solarScrimStyle(props: SolarScrimProps = {}): Style {
  const p: Record<string, unknown> = { ...solarScrimDefaults };
  for (const [k, v] of Object.entries(props)) if (v !== undefined) p[k] = v;
  const key = `default`;
  const s = solarScrimStyles as unknown as {
    reset: Style;
    root: Style;
...
export type SolarScrimParts = Record<
  string,
  string | number | boolean | Glyph | null
>;
type Parts = SolarScrimParts;
type Layered = {
  base: Parts;
 ...
 */
export function solarScrimCompose(
  props: SolarScrimProps = {},
  ...
  for (const [layer, c] of Object.entries(
    solarScrimComposition as unknown as Record<string, Layered>,
  )) {
...
export const solarScrimTree: Record<string, string[]> = {};

/** Each slot's layer, to the slot it is: its class is `SolarScrim-<slot>`, public. */
export const solarScrimSlots: Record<string, string> = {};
