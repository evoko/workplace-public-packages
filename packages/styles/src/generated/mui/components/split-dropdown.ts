// SOLAR Split Dropdown recipe for MUI. Generated by @bwp-web/codegen from spec/components/split dropdown.json. Do not edit.
...
export type SolarSplitDropdownProps = Record<never, never>;

export const solarSplitDropdownDefaults = {} as const;

/** Style by layer and state: `root` is the base, then per size, per appearance, and per size and appearance together. */
export const solarSplitDropdownStyles = {
  reset: {
    display: 'flex',
    position: 'relative',
    boxSizing: 'border-box',
    flexShrink: '0',
    '& .SolarSplitDropdown-box': {
      display: 'flex',
    },
    '& .SolarSplitDropdown-text': {
      whiteSpace: 'nowrap',
    },
...
export const solarSplitDropdownComposition = {
  root: {
    base: {
      present: true,
    },
...
export function solarSplitDropdownStyle(
  props: SolarSplitDropdownProps = {},
): Style {
  const p: Record<string, unknown> = { ...solarSplitDropdownDefaults };
  for (const [k, v] of Object.entries(props)) if (v !== undefined) p[k] = v;
  const key = `default`;
  const s = solarSplitDropdownStyles as unknown as {
    reset: Style;
    root: Style;
...
export type SolarSplitDropdownParts = Record<
  string,
  string | number | boolean | Glyph | null
>;
type Parts = SolarSplitDropdownParts;
type Layered = {
  base: Parts;
 ...
 */
export function solarSplitDropdownCompose(
  props: SolarSplitDropdownProps = {},
  ...
  for (const [layer, c] of Object.entries(
    solarSplitDropdownComposition as unknown as Record<string, Layered>,
  )) {
