// SOLAR Alert Small recipe for MUI. Generated by @bwp-web/codegen from spec/components/alert small.json. Do not edit.
...
export type SolarAlertSmallType =
  'default' | 'info' | 'success' | 'warning' | 'danger';
export type SolarAlertSmallVariant = 'filled' | 'outlined';

export interface SolarAlertSmallProps {
  type?: SolarAlertSmallType;
  variant?: SolarAlertSmallVariant;
}

export const solarAlertSmallDefaults = {
  type: 'success',
  variant: 'filled',
} as const;

/** Style by layer and state: `root` is the base, then per size, per appearance, and per size and appearance together. */
export const solarAlertSmallStyles = {
  reset: {
    display: 'inline-flex',
    position: 'relative',
    ...
    '& .SolarAlertSmall-text': {
      whiteSpace: 'nowrap',
    },
    '& .SolarAlertSmall-glyph': {
      display: 'block',
      overflow: 'visible',
      flexShrink: '0',
    },
    '& .SolarAlertSmall-drawnIcon': {
      flexShrink: '0',
    },
...
export const solarAlertSmallComposition = {
  root: {
    base: {
      present: true,
    },
...
export function solarAlertSmallStyle(props: SolarAlertSmallProps = {}): Style {
  const p: Record<string, unknown> = { ...solarAlertSmallDefaults };
  for (const [k, v] of Object.entries(props)) if (v !== undefined) p[k] = v;
  const key = `type=${p.type}, variant=${p.variant}`;
  const s = solarAlertSmallStyles as unknown as {
    reset: Style;
    root: Style;
...
export type SolarAlertSmallParts = Record<
  string,
  string | number | boolean | Glyph | null
>;
type Parts = SolarAlertSmallParts;
type Layered = {
  base: Parts;
 ...
 */
export function solarAlertSmallCompose(
  props: SolarAlertSmallProps = {},
  ...
  for (const [layer, c] of Object.entries(
    solarAlertSmallComposition as unknown as Record<string, Layered>,
  )) {
...
export const solarAlertSmallTree: Record<string, string[]> = {
  root: ['statusIndicator', 'content', 'action'],
  content: ['title', 'description'],
};

/** Each slot's layer, to the slot it is: its class is `SolarAlertSmall-<slot>`, public. */
export const solarAlertSmallSlots: Record<string, string> = {
  title: 'title',
