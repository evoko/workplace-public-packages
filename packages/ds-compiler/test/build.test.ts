...
import { build, buildIR, hashableConfig } from '../src/build.js';
...
describe('buildIR on the mini fixture', () => {
  ...
  it('hashes the config without `rendered`, and with everything else', () => {
    const hashWith = (config: Record<string, unknown>): string => {
      const root = makeRoot({});
      cpSync(FIXTURE_MINI, root, { recursive: true });
      writeFileSync(
      ...
      const { ir, diagnostics } = buildIR(root);
      expect(diagnostics.errors).toEqual([]);
      return ir!.meta.sourceHash;
    };
    const base = {
      name: 'Fictional',
      prefix: 'fx',
    ...
    // `rendered` only names a command for `verify --rendered`; it reaches no
    // generated byte, so it must not make every generated file drift.
    expect(hashWith({ ...base, rendered: { cwd: '..', command: 'a' } })).toBe(
      hashWith({ ...base, rendered: { cwd: '../..', command: 'b' } }),
    );
    expect(hashWith({ ...base, rendered: { cwd: '..', command: 'a' } })).toBe(
      hashWith(base),
    );
    // Anything a generator reads still changes it.
    expect(
      hashWith({ ...base, targets: { tailwind: { outDir: '../out' } } }),
    ).not.toBe(hashWith(base));
  });

  it('hashes the config through formatting changes, not around them', () => {
    const pretty =
      '{\n  "name": "Fictional",\n  "prefix": "fx",\n  "modes": ["light"],\n  "defaultMode": "light"\n}\n';
    const reshuffled =
      '{\n    "defaultMode": "light",\n    "modes": [ "light" ],\n    "prefix": "fx",\n    "name": "Fictional"\n}\n';
    expect(hashableConfig(reshuffled)).toBe(hashableConfig(pretty));
    // Not a blanket "any text is equal" rule: values still count.
    expect(hashableConfig(pretty.replace('"fx"', '"gx"'))).not.toBe(
      hashableConfig(pretty),
    );
    // Array order is meaningful, so it survives canonicalisation.
    expect(hashableConfig('{"modes": ["light", "dark"]}')).not.toBe(
      hashableConfig('{"modes": ["dark", "light"]}'),
    );
  });

  it('falls through to the raw text when the config is not a JSON object', () => {
    // Defensive: loadConfig rejects these before the hash is reached, so the
    // fall-through must simply not throw and must stay deterministic.
    expect(hashableConfig('not json {')).toBe('not json {');
    expect(hashableConfig('[1, 2]')).toBe('[1, 2]');
    expect(hashableConfig('null')).toBe('null');
    // And the build still reports the unreadable config, writing nothing.
    const root = makeRoot({ 'ds.config.json': 'not json {' });
