...
/**
 * The config as it enters the source hash: `rendered` removed, keys sorted.
 *
 * `rendered` only tells `bwp-ds verify --rendered` which command to run; it
 * cannot reach a single generated byte, so changing it must not mark every
 * generated file as drifted. Canonicalising the rest also means reformatting
 * ds.config.json (indentation, key order) no longer invalidates the hash,
 * while every value the generators read still does. Unparseable text is
 * returned as-is: loadConfig has already reported it.
 */
export function hashableConfig(raw: string): string {
  let parsed: unknown;
  try {
  ...
  const { rendered: _rendered, ...rest } = parsed as Record<string, unknown>;
...
export function buildIR(rootDir: string): BuildResult {
  ...
  sources.push({
    path: CONFIG_FILE,
    contents: hashableConfig(readFileSync(join(rootDir, CONFIG_FILE), 'utf8')),
  });
