...
2. **Never edit generated files:** anything under
   `packages/styles-tailwind/src/generated/` (and any future `generated/`
   directory), `packages/styles-css/design.ir.json`,
   `packages/styles-css/src/index.css`, and `docs/design-system/coverage.md`.
   They are regenerated by `bwp-ds build`, `bwp-ds generate`, and
   `bwp-ds verify`, and CI fails on drift.
...
| Path                                                        | Package                          | Role                                                                                                                                            |
| ----------------------------------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/ds-compiler`                                      | `@bwp-web/ds-compiler` (private) | Parser, IR, `bwp-ds` CLI: lint, build, scaffold, generate, verify. Tailwind target implemented; MUI, Flutter, and a story generator come later. |
...
5. `npm run ds -- lint`, then `npm run ds -- build`. The build regenerates
   ...
   exclusion) or verify fails with `DS-E082`.
...
`DS-E07x` generated files, `DS-E08x` generated output and verification,
`DS-W00x` warnings.
