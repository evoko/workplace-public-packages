...
2. **Never edit generated files:** anything under
   ...
   (and any future `generated/` directory), `packages/styles-css/design.ir.json`,
...
| Path                                                        | Package                          | Role                                                                                                                                                                       |
| ----------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/ds-compiler`                                      | `@bwp-web/ds-compiler` (private) | Parser, IR, `bwp-ds` CLI: lint, build, scaffold, generate, verify. Tailwind and MUI targets implemented; Flutter and the story generator come later.                       |
...
`verify` adds `steps` (`lint`, `drift`, `roundtrip`, `coverage`, each `pass`,
`fail`, or `skipped`) and `coverageFile`; `generate` adds `wrote` and
`removed`.
