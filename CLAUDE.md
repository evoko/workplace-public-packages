...
Spinner, F1's display primitives, from StatusIndicator to Tree Indent, and F3's selection
controls, from Checkbox to Segmented Control, are the components so far; Cursor is left out by
decision, `spec/overlay/excluded.yaml`).
...
- Packages: `@bwp-web/styles` (tokens and theme, generated), `@bwp-web/assets` (the 340 SOLAR
  ...
  FAB, BackButton, SplitButton, Link, Spinner, the F1 display primitives and the F3 selection
  controls so far), all at
