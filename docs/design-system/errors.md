...
| DS-E085 | A component the MUI target cannot render as a React component: a state outside the pseudo-class and ARIA sets, a slot element that is not an HTML or SVG tag, a void element, or an SVG element outside an `svg` root, or a slot named like a reserved prop, axis, or state | Change the manifest, or set `targets.mui` to `{ "excluded": "<reason>" }`.                                        |
