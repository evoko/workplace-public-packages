...
| `preview`     | no       | Free-form `string: string` map: the text the generated compare stories render. `preview.label` is the component's label text (a button's text); it falls back to `displayName`. Every other key is the text content of the slot it names, falling back to the slot's own name, and a key matching no slot is ignored — keys are not validated against `slots`. Values are plain text, or an icon name for an icon slot. Nothing here affects the CSS or the targets.                                                                                                                                     |
