...
component CSS and manifests) sorted by path. The token and component files
enter it as their exact bytes; `ds.config.json` enters it canonicalised —
parsed, keys sorted, whitespace and key order ignored — and without the
`rendered` key, which only tells `verify --rendered` which command to run and
reaches no generated byte. So reindenting the config, or pointing `rendered`
somewhere else, does not mark every generated file as drifted, while any
value a generator reads still does. Two builds of identical sources produce
identical files; the JSON has sorted keys and a trailing newline. All object
keys in `design.ir.json` are sorted alphabetically; only arrays carry
meaningful order.
