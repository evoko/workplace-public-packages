...
| CI "Check generated files are up to date"                | `design.ir.json` and `src/index.css` are rewritten by `npm run build`, `docs/design-system/coverage.md` by `npm run verify`, and the diff catches a stale committed copy of any of them; `packages/styles-tailwind/src/generated` and `packages/styles-mui/src/generated` are guarded by `verify`'s drift check (`DS-E080`), not by the diff. | CI      |
