...
// SOLAR Web's own Layout collection (grid columns, margins, gutters, breakpoints). It is not
// a Foundations collection, but a web theme needs it, so it is folded into the contract when
// the capture exists. Its aliases point into the Foundations primitives.
const layoutPath = join(
  ...
  'solar-web',
  'tokens',
  'layout-variables.json',
);
const layoutInv = existsSync(layoutPath)
...
// layout (SOLAR Web local collection, single mode)
if (layoutInv)
  for (const [name, v] of Object.entries(layoutInv.variables)) {
    const doc = 'layout.' + name.replace(/\//g, '.');
    const isAlias = typeof v.value === 'string';
    const raw = isAlias ? resolveAlias(v.value) : v.value;
    entries.push({
      collection: 'Layout',
      tier: 'semantic',
      source: 'solar-web',
      ...
      doc,
      css: cssVar(doc),
      value: /columns/.test(name) ? String(raw) : `${raw}px`,
      ...(isAlias ? { alias: v.value.slice(1, -1) } : {}),
    });
  }
...
const contract = {
  ...
  layoutSource: layoutInv ? layoutInv.source : null,
  variables: entries,
...
if (layoutInv) {
  css +=
    '  /* layout (SOLAR Web collection: grid columns are unitless counts) */\n';
  for (const e of entries.filter((e) => e.collection === 'Layout'))
    css += line(e.css, e.value);
}
css += '  /* shadows (Light) */\n';
...
const grammar = {
  ...
  patterns: {
    ...
    'layout (SOLAR Web)':
      '^layout\\.(grid\\.(columns|margin|gutter)|breakpoint)\\.(xs|sm|md|lg|xl)$',
    motion:
