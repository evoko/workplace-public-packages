...
A standalone component (`components[]`, one Figma drew with no variants) has its `name`, `id`,
`description`, `props`, `nodeCount` and `tree`, and, where it hides layers, `hiddenPaths`: each
one by path, into the instances the tree does not descend into, as a set's variants record them
(Calendar Toolbar's Segmented Control, its label hidden). Data fetched before 2026-09-25 has none.
...
| `dashes`                                           | For a dashed stroke: Figma's `strokeDashes`, each dash's length and the gap after it (`[3, 3]`)                                                                                                                                                |
| `arc`                                              | For a part of an ellipse: its `start` and `end` angles (radians) and its `inner` hole, a fraction of its radius                                                                                                                                |
