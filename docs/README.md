...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Human-written Foundations reference (18 chapters + agent quick-reference)                                                                                                                                                                                                      | Every component but Button and Spinner. The next ten (Text Input, Checkbox, Tabs, Icon Button, Button Group, Tag, Card, Dialog, Sparkline) are milestone 3b-2, on the machinery 3b-1 built and proved on Button. |
| Foundations token inventory as JSON, and three files derived from it by a script                                                                                                                                                                                               | The rest of the developer tweak loop: `solar:explain` and the Storybook review surface are designed in [the docs-to-code spec](superpowers/specs/2026-09-21-solar-docs-to-code-design.md) and not built (3c).    |
...
| **Code generation**: `npm run solar:codegen` emits tokens to CSS, MUI, Tailwind and Flutter, the icons and logos to React, raw SVG and Flutter, and the Button and Spinner recipes to MUI and Flutter, with parity suites proving the targets agree                            |
| **Components**: SOLAR Button and Spinner for React (`@bwp-web/components`, on MUI) and for Flutter (`SolarButton`, `SolarSpinner`), each overridable through a reviewed overlay in `spec/overlay/`, and checked on both platforms against what Figma draws, variant by variant |                                                                                                                                                                                                                  |
