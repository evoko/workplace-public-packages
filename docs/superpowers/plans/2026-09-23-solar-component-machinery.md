...
**Done 2026-09-23.** One change from the text above: an excused entry keeps *Figma's* value, not
the recipe's, marked `excused` with the finding (and its decision, if one was taken). Taking the
recipe's value would have made the oracle read the recipe, which the last test forbids; the
visual checks skip excused entries instead, so the report still shows the gap. `spec/verify/` is
generated by `solar:codegen` beside `spec/components/`, pruned like it, and checked by CI's
rebuild. Button: 108 variants, each reachable once through the API (props plus platform state);
18 excused entries, exactly the variants Button's three open findings name. Spinner: 6 variants;
the default indicator's colour is unreadable (`null`) and excused by the overlay's `set`, which
now also records itself as that finding's decision in the governance report. Numbers are what
Figma draws; letter spacing is in pixels (tokens mix `-2%` and `-0.32px`); decoration is spelled
as CSS. Instances record only what the parent decides: an icon's colour, a child's variant, a
fixed size. 453 tests and lint pass; the rebuild is deterministic.
