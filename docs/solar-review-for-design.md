...
4. **Sizes with no variable** — Counter's badge, Avatar, ProgressBar, the table and tree rows, a
   ...
   Input's cells, Number Input's side stepper), Context Menu Item's 36, and a menu's ~300px
   maximum height, which Dropdown Menu's description asks for (we cap at one raw 300): one family
   of size variables? Section 6.
