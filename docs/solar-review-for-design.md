...
| Question                                                                                                                                                                                                                                                                                                                                                                                         | What we do meanwhile                                 |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
| **Leading trim**: every SOLAR text is trimmed to its cap height (Figma's leading trim), so a text box is shorter than its line. Neither platform trims a line the same way, and a row sized to trimmed text cannot hold full lines: Agenda Row's 64 and 32 hold 76 and 44. Should code trim text (CSS's `text-box`, and an equivalent in Flutter), or should components be sized for full lines? | Full lines; Agenda Row grows to hold them.           |
| **The Agenda Row's dot** (**Agenda Row** · [6651:138](https://figma.com/design/OGvmMNnywH7JWDyEhOzjcc?node-id=6651-138)): its description says its colour "should match the source event", and Figma draws category 06 only. Should it take an Event Chip's `category`, as the chip does?                                                                                                        | The app gives its colour; category 06 without one.   |
| **All-Day Bar's colour and span** (**All-Day Bar** · [6644:50](https://figma.com/design/OGvmMNnywH7JWDyEhOzjcc?node-id=6644-50)): it has no category, drawn in category 06 only, where an Event Chip has eight; and its description says the span sets the corners so segments fuse, where every span is drawn alike.                                                                            | Category 06; every span alike, as drawn.             |
...
1. **Sizes with no variable** — the heights on no `size/control` step (FAB's 44 and 56, Banner's 44,
   ...
   hour's 48 and 32, the time rail's 56, an agenda row's time column (36, 76) and 8px dot: one
...
9. **Overlays** — Tooltip's delay, a Split Dialog on a narrow viewport, and Coachmark's close and
   title. Section 2.
10. **Leading trim** — every SOLAR text is trimmed to its cap height, which neither platform draws
    alike: should code trim text, or size components for full lines? Section 2.
11. **Calendar** — the Agenda Row's dot and the All-Day Bar's colour (an Event Chip's category?), a
    bar's span corners, Event Chip's densities, Weekday Header's today and the toolbar's New Event.
    Section 2.
